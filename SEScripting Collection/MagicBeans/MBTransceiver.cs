...
class Transceiver : MyGridProgram
{
    #region in-game
    static class Names
    {
            ...
            case CommunicationModel.CommunicationScopes.INTERNAL:
                {
                    ...
                    var action = antenna.GetActionWithName("TransmitMessage");
                    var reader = new ListReader<TerminalActionParameter>(
                        new List<TerminalActionParameter>() { 
                            TerminalActionParameter.Get(output) 
                        }
                    );
                    action.Apply(antenna, reader);
                }                
                break;
